...
void setpixel(uint8_t *buff, uint8_t x, uint8_t y, uint8_t color) {
	// assume x and y are valid inputs, meaning x and y are within range
	uint8_t line_position = x;
	uint8_t height = y / 8;
	uint8_t reminder = y % 8;
	buff[128*height+line_position] |= (int)(pow(2, 7 - reminder) + 0.5);
}

// function to clear a single pixel
void clearpixel(uint8_t *buff, uint8_t x, uint8_t y) {
	// assume x and y are valid inputs, meaning x and y are within their corresponding range
	uint8_t line_position = x;
	uint8_t height = y / 8;
	uint8_t reminder = y % 8;
	buff[128*height+line_position] &= ~((int)(pow(2, 7 - reminder) + 0.5));
...
void drawline(uint8_t *buff,uint8_t x0, uint8_t y0, uint8_t x1, uint8_t y1,uint8_t color) {
	// assume the inputs are all valid and are within range
...
void drawcircle(uint8_t *buff,uint8_t xc, uint8_t yc, uint8_t r,uint8_t color) {
	 int x,y,Pk;
	 x=0;
	 y=r;
	 ...
	 {
		 y = Pk < 0  ? y : y-1;
		 Pk = Pk < 0 ? Pk+(4*x)+6 : Pk+(4*(x-y)+10);
		 
		 setpixel(buff,xc+x,yc-y,7);
		 setpixel(buff,xc-x,yc-y,7);
		 setpixel(buff,xc+x,yc+y,7);
		 setpixel(buff,xc-x,yc+y,7);
		 setpixel(buff,xc+y,yc-x,7);
		 setpixel(buff,xc-y,yc-x,7);
		 setpixel(buff,xc+y,yc+x,7);
		 setpixel(buff,xc-y,yc+x,7);
...
void fillcircle(uint8_t *buff,uint8_t xc, uint8_t yc, uint8_t r,uint8_t color) {
	int x,y,Pk;
	x=0;
	y=r;
	setpixel(buff,xc+x,yc-y,1);
	...
	for(x=0;x<=y;x++)
	{
		...
		drawline(buff, xc+x, yc-y, xc+x, yc+y, 0);
		drawline(buff, xc-x, yc-y, xc-x, yc+y, 0);
		drawline(buff, xc+y, yc-x, xc+y, yc+x, 0);
		drawline(buff, xc-y, yc-x, xc-y, yc+x, 0);
	}
}
