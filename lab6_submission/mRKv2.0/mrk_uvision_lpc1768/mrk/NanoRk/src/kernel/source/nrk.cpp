...
int8_t nrk_TCB_init (nrk_task_type *Task, NRK_STK *ptos, NRK_STK *pbos, uint16_t stk_size, void *pext, uint16_t opt)
{
    ...
    nrk_task_TCB[Task->task_ID].cpu_reserve= _nrk_time_to_ticks(&(Task->cpu_reserve));
		...
		/*
		//Check if user assigned relative deadline, by default the value is the period.
		if(Task->relative_deadline.secs == 0 && Task->relative_deadline.nano_secs == 0)
		{
    nrk_task_TCB[Task->task_ID].r_deadline = _nrk_time_to_ticks(&(Task->period));
    }
		else
		{
		nrk_task_TCB[Task->task_ID].r_deadline = _nrk_time_to_ticks(&(Task->relative_deadline));
		}
		*/
